...
class UnfoldingKGE(nn.Module):
    ...
    def forward_small(self, graph, x):
        h = self.mlp(x)
        '''pos_edges = graph.edges()
        num_edges = pos_edges[0].shape[0]
        dropout = 0.1
        mask = torch.zeros(pos_edges[0].shape, dtype = bool)
        num_samples = int(num_edges * dropout)
        indices = torch.randperm(num_edges)[:num_samples]
        mask[:] = True
        mask[indices] = False
        sampled_edges = (pos_edges[0][mask], pos_edges[1][mask])
        sampled_graph = dgl.graph(sampled_edges, device = "cpu", num_nodes =graph.number_of_nodes())
        sampled_graph = dgl.to_bidirected(sampled_graph, copy_ndata = True).to(x.device)'''
